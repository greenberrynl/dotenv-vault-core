...
function config(options) {
  ...
  const vault = configVault(options)

  if (options.save) {
    // create dotenf file from object
    const dotenvFilename = options.save !== "true" && options.save !== "1" ? options.save : ".env"
    const saveLocation = path.resolve(process.cwd(), dotenvFilename)
    const items = Object.entries(vault.parsed).map(([key, value]) => `${key}=${value}`)
    fs.writeFileSync(saveLocation, items.join('\n'))
  }
